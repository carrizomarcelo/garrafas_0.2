...
QUnit.test('bindClickHandler', (assert) => {
  ...
  SwalWithoutAnimation.mixin({
    toast: true
  }).bindClickHandler('data-swal-toast-templates')

  const template = document.createElement('template')
  template.id = 'my-templates-for-declarative-triggering'
  template.innerHTML = '<swal-title>Are you sure?</swal-title>'
  document.body.appendChild(template)

  const buttonTriggerPopup = document.createElement('button')
  buttonTriggerPopup.setAttribute('data-swal-template', '#my-templates-for-declarative-triggering')
  document.body.appendChild(buttonTriggerPopup)

  const buttonTriggerToast = document.createElement('button')
  buttonTriggerToast.setAttribute('data-swal-toast-templates', '#my-templates-for-declarative-triggering')
