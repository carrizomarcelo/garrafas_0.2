...
QUnit.test('templates as HTMLTemplateElement', (assert) => {
  const template = document.createElement('template')
  template.id = 'my-templates'
  template.innerHTML = `
  ...
  SwalWithoutAnimation.fire({
    template: document.querySelector('#my-templates'),
  })
...
QUnit.test('templates as string', (assert) => {
  const template = document.createElement('template')
  template.id = 'my-templates-string'
  template.innerHTML = '<swal-title>Are you sure?</swal-title>'
  document.body.appendChild(template)
  const mixin = SwalWithoutAnimation.mixin({
    title: 'this title should be overriden by templates',
  })
  mixin.fire({
    template: '#my-templates-string',
  })
  assert.equal(Swal.getTitle().textContent, 'Are you sure?')
})
...
QUnit.test('should throw a warning when attempting to use unrecognized elements and attributes', (assert) => {
  ...
  const template = document.createElement('template')
  template.id = 'my-templates-with-unexpected-attributes'
  template.innerHTML = `
  ...
  document.body.appendChild(template)
  const mixin = SwalWithoutAnimation.mixin({
    imageAlt: 'this alt should be overriden by templates',
  })
  mixin.fire({
    imageWidth: 200, // user param should override <swal-image width="100">
    template: '#my-templates-with-unexpected-attributes',
  })
  assert.equal(Swal.getImage().src, 'https://sweetalert2.github.io/images/SweetAlert2.png')
